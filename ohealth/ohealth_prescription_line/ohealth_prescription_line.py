...
class OHealthPrescriptionLine(osv.Model):
    _name = 'ohealth.prescription.line'



    def _get_medicament(self, cr, uid, ids, name, args, context=None):
        print '_get_medicament', name, args, context, ids
        medication_obj = self.pool.get('ohealth.medication.template')
        result = {}
...
#        return res
    ...
    _columns = {
        'name': fields.many2one('ohealth.prescription.order', string='Prescription ID', ),
        'template': fields.many2one('ohealth.medication.template', string='Medication', ),
        'indication': fields.many2one('ohealth.pathology', string='Indication', help='Choose a disease for this medicament from the disease list. It'\
        ...
        'form': fields.many2one('ohealth.drug.form', string='Form', help='Drug form, such as tablet or gel'),
        'route': fields.many2one('ohealth.drug.route', string='Route', help='Drug form, such as tablet or gel'),
        'common_dosage': fields.many2one('ohealth.medication.dosage', string='Frequency', help='Drug form, such as tablet or gel'),
