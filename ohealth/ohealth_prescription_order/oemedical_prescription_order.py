...
class OHealthPrescriptionOrder(osv.Model):
    _name='ohealth.prescription.order'

    _columns={
        'patient_id': fields.many2one('ohealth.patient', string='Patient', required=True),
        'pregnancy_warning': fields.boolean(string='Pregancy Warning', readonly=True),
        'notes': fields.text(string='Prescription Notes'),
        'prescription_line': fields.one2many('ohealth.prescription.line', 'name', string='Prescription line',),
        ...
        'physician_id': fields.many2one('ohealth.physician', string='Prescribing Doctor',  required=True),
...
#        wf_service.trg_validate(uid, 'ohealth.prescription.order', ids[0], 'prescription_sent', cr)
        datas = {
                 'model': 'ohealth.prescription.order',
                 'ids': ids,
