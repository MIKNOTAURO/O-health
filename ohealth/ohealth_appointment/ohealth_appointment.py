...
class OHealthAppointment(osv.Model):
    ...
    _columns = {
        ...
        'doctor': fields.many2one('ohealth.physician',
        ...
        'speciality': fields.many2one('ohealth.specialty',
                                      string='Specialty', 
        ...
        'state': fields.selection([
            ...
            string='State'),
        'history_ids' : fields.one2many('ohealth.appointment.history','appointment_id_history','History lines', states={'start':[('readonly',True)]}),

    }
    
    _defaults = {
        'name': lambda obj, cr, uid, context: 
            obj.pool.get('ir.sequence').get(cr, uid, 'ohealth.appointment'),
        'duration': 30.00,
        'urgency': 'a',
        'state': 'draft',
    ...
    def create(self, cr, uid, vals, context=None):
        val_history = {}
        ait_obj = self.pool.get('ohealth.appointment.history')
        ...
        return super(OHealthAppointment, self).create(cr, uid, vals, context=context)
...
class OHealthAppointment_history(osv.Model):
    _name = 'ohealth.appointment.history'

    _columns = {
        'appointment_id_history' :  fields.many2one('ohealth.appointment','History', ondelete='cascade'),
