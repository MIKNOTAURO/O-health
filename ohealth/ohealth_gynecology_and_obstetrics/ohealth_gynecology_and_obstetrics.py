...
class PatientPregnancy(osv.Model):
    ...
    _name = 'ohealth.patient.pregnancy'
...
#            else:
    ...
    _columns = {
                'name' : fields.many2one('ohealth.patient', 'Patient ID'),
                ...
                'prenatal_evaluations' : fields.one2many('ohealth.patient.prenatal.evaluation', 'name', 'Prenatal Evaluations'),
                'perinatal' : fields.one2many('ohealth.perinatal', 'name', 'Perinatal Info'),
                'puerperium_monitor' : fields.one2many('ohealth.puerperium.monitor', 'name', 'Puerperium monitor'),
...
class PrenatalEvaluation(osv.Model):
    ...
    _name = 'ohealth.patient.prenatal.evaluation'
...
#            return gestational_age.days
    ...
    _columns = {
            'name' : fields.many2one('ohealth.patient.pregnancy', 'Patient Pregnancy'),
            'evaluation' : fields.many2one('ohealth.patient.evaluation', 'Patient Evaluation', readonly=True),
...
class PuerperiumMonitor(osv.Model):
    ...
    _name = 'ohealth.puerperium.monitor'
    _description = 'Puerperium Monitor'

    _columns = {
        'name' : fields.many2one('ohealth.patient', string='Patient ID'),
...
class PerinatalMonitor(osv.Model):
    ...
    _name = 'ohealth.perinatal.monitor'
    _description = 'Perinatal monitor'
    _columns = {
            'name' : fields.many2one('ohealth.patient', string='patient_id'),
...
class OhealthPerinatal(osv.Model):

    _name = 'ohealth.perinatal'
    _description =  'Perinatal Information'
    _columns={
            'name' : fields.many2one('ohealth.patient', string='Perinatal Infomation'),
            ...
            'monitoring' : fields.one2many('ohealth.perinatal.monitor', 'name', string='Monitors'),
            'puerperium_monitor' : fields.one2many('ohealth.puerperium.monitor', 'name','Puerperium monitor'),
...
#            'currently_pregnant' : fields.function( _get_pregnancy_info , string='Pregnant' , type='boolean' ),
            ...
            'menstrual_history' : fields.one2many('ohealth.patient.menstrual_history', 'name', 'Menstrual History'),
            'mammography_history' : fields.one2many('ohealth.patient.mammography_history', 'name', 'Mammography History'),
            'pap_history' : fields.one2many('ohealth.patient.pap_history', 'name', 'PAP smear History'),
            'prenatal_evaluations' : fields.one2many('ohealth.patient.prenatal.evaluation', 'name', 'Prenatal Evaluations'),
            'colposcopy_history' : fields.one2many('ohealth.patient.colposcopy_history', 'name', 'Colposcopy History'),
            'pregnancy_history' : fields.one2many('ohealth.patient.pregnancy', 'name', 'Pregnancies'),
            }
...
class PatientMenstrualHistory(osv.Model):
    ...
    _name = 'ohealth.patient.menstrual_history'
    _description =  'Menstrual History'
    _columns={
            'name' : fields.many2one('ohealth.patient', 'Patient', readonly=True, required=True),
            'evaluation' : fields.many2one('ohealth.patient.evaluation', 'Evaluation'),
...
class PatientMammographyHistory(osv.Model):
    ...
    _name = 'ohealth.patient.mammography_history'
...
class PatientPAPHistory(osv.Model):
    ...
    _name = 'ohealth.patient.pap_history'
...
class PatientColposcopyHistory(osv.Model):
    ...
    _name = 'ohealth.patient.colposcopy_history'
